...
include "../utils/poseidon.util.circom";
...
*/
template CredentialAtomicQuerySigV2(IdOwnershipLevels, IssuerLevels, valueArraySize) {
    ...
    */
    ...
    q.operator <== operator;
    component poseidonUtil = PoseidonUtil(valueArraySize);
    for(var i = 0; i < valueArraySize; i++) {  
        q.value[i] <== value[i];
        poseidonUtil.in[i] <== value[i];
    }
    valueHash <== poseidonUtil.out;
    q.out === 1;
}
