...
include "../lib/utils/valuesHasher.circom";
...
*/
template CredentialAtomicQueryMTPOnChain(IssuerLevels, ClaimLevels, valueArraySize) {
    ...
    */
    ...
    component valuesHasher = ValuesHasher(valueArraySize);
    for (var i = 0; i < valueArraySize; i++) { 
        query.value[i] <== value[i];
        valuesHasher.in[i] <== value[i];
    }

    valuesHash <== valuesHasher.out;
