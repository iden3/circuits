...
func Test_IssueClaimToProfile_2(t *testing.T) {
	...
	desc := "ON Chain: UserID != Subject. UserProfile out. User nonce = 10. Claim issued on Profile (subject nonce = 0) (Merklized claim)"
	isUserIDProfile := true
	isSubjectIDProfile := false
...
func Test_IssueClaimToProfile_3(t *testing.T) {
	...
	desc := "ON Chain: UserID == Subject. UserProfile out. User nonce = 10. Claim issued on Profile (subject nonce = 999) (Merklized claim)"
	isUserIDProfile := true
	isSubjectIDProfile := true
...
func Test_RegularClaim(t *testing.T) {
	...
	desc := "ON Chain: UserID == Subject. UserID out. User nonce = 0, Subject nonce = 0 claim issued on userID (Claim)"
	isUserIDProfile := false
	isSubjectIDProfile := false
...
func Test_JSON_LD_Proof_non_inclusion(t *testing.T) {
	...
	desc := "ON Chain: JSON-LD proof non inclusion. UserID = Subject. UserID out. User nonce = 0, " +
