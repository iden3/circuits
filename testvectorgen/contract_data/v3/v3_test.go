...
func generateStateTransitionData(t *testing.T, nextState bool, primaryPK, secondaryPK, desc, fileName string, isSubjectIDProfile bool, isEthBased bool) (*big.Int, *big.Int) {
	...
	authNonRevMTProof, nodeAuxNonRev := primaryEntity.ClaimRevMTPLevels(t, primaryEntity.AuthClaim, utils.IdOwnershipLevels)
...
func generateData(t *testing.T, desc string, gistData []*gistData, userFirstState bool, userSecondState bool, issuetGenesisState bool, issuerSecondState bool, fileName string, testProofType verifiable.ProofType, isBJJAuthEnabled int) {
	...
	valueInput := utils.PrepareStrArray([]string{"20010101"}, utils.MaxValueArraySize)
	...
	if isJSONLD {
		...
		claimPathMtp = utils.PrepareStrArray([]string{}, utils.ClaimLevels)
		claimPathMtpNoAux = "0"
		claimPathMtpAuxHi = "0"
		claimPathMtpAuxHv = "0"
		claimPathKey = "0"
	...
	issuerAuthClaimMtp, _ = issuer.ClaimMTPLevels(t, issuer.AuthClaim, utils.IssuerLevels)

	if !issuetGenesisState {
		issuer.AddClaim(t, claim)
	}

	issuerClaimMtp, _ = issuer.ClaimMTPLevels(t, claim, utils.IssuerLevels)
	require.NoError(t, err)
	...
	issuerClaimNonRevMtp, issuerClaimNonRevAux := issuer.ClaimRevMTPLevels(t, claim, utils.IssuerLevels)

	issuerAuthClaimNonRevMtp, issuerAuthClaimNodeAux := issuer.ClaimRevMTPLevels(t, issuer.AuthClaim, utils.IssuerLevels)
	...
	if testProofType == verifiable.BJJSignatureProofType {
		...
		issuerAuthClaimMtp = utils.PrepareStrArray([]string{}, utils.IssuerLevels)
	...
	gisTree, err := merkletree.NewMerkleTree(context.Background(), memory.NewMemoryStorage(), utils.OnChainLevels)
	require.Nil(t, err)
	...
	// user
	if isBJJAuthEnabled == 1 {
		authMTProof = user.AuthMTPStrignLevels(t, utils.IdOwnershipLevels)
		userAuthNonRevMTProof, userNodeAuxNonRev = user.ClaimRevMTPLevels(t, user.AuthClaim, utils.IdOwnershipLevels)
		...
		userAuthNonRevMTProof = utils.PrepareSiblingsStr(emptyArr, utils.IssuerLevels)
